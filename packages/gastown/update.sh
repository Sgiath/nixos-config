...
#   ./update.sh 0.3.0     # Update to specific version
...
if [[ -n "${1:-}" ]]; then
  VERSION="$1"
  echo "==> Updating gastown to specified version ${VERSION}"
else
  echo "==> Fetching latest gastown version from GitHub..."
  LATEST_TAG=$(curl -s "https://api.github.com/repos/steveyegge/gastown/tags" | jq -r '.[0].name')
...
SRC_JSON=$(nix run nixpkgs#nix-prefetch-github -- steveyegge gastown --rev "v${VERSION}" 2>/dev/null)
...
echo "==> Done! Updated gastown to version ${VERSION}"
echo ""
echo "Next steps:"
echo "  1. Test the build: nix build '.#gastown'"
