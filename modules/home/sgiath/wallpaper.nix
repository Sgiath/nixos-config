  ...
  config = lib.mkIf config.programs.hyprland.enable {
    wayland.windowManager.hyprland = {
      ...
      settings = {
        ...
        bind = [
          "$mod, W, exec, dbus-run-session ${pkgs.ungoogled-chromium}/bin/chromium --kiosk --user-data-dir=/tmp/chrome-temp --incognito --no-first-run --ozone-platform=x11 --class=nasa ${nasa_url}"
        ];
