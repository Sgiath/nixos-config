...
description: Rename and move movies, TV series, and music so Jellyfin identifies them. Use when organizing a media library, fixing Jellyfin matching, renaming downloads, or moving files into Movies/Shows/Music folders.
---

# Jellyfin naming

Organize media the way Jellyfin's scanner expects. Wrong folders matter more than pretty names. A movie sitting loose in `Movies/`, a show without `Season 01`, or two albums in one folder will mis-identify even if the filenames look fine.

Official docs (read the matching reference before renaming a type):
...
- Provider IDs: <https://jellyfin.org/docs/general/server/metadata/identifiers>
- Details: [movies.md](references/movies.md), [series.md](references/series.md), [music.md](references/music.md), [shared.md](references/shared.md)

This skill covers **movies, series, and music**. Do not invent book/audiobook/music-video layouts from it.

## Hard rules

1. **Never use** these characters in any path component: `< > : " / \ | ? *`
2. **One movie per folder. One album per folder. Shows = series folder → season folders → episodes.**
3. Movie/show names should match the metadata provider title. Year and provider IDs are optional but preferred.
4. Movie video basename **must equal** the movie folder name (including year and IDs) before any version/part suffix.
5. Season folders are `Season 01`, never `S01` / `SE01`. Pad season numbers.
6. Do not mix versions (` - 1080p`) with stacked parts (`-cd1`). Jellyfin does not support both on the same item.
7. Move every sidecar with its media file: subtitles, audio tracks, NFO, lyrics, thumbs.
8. Do not remux, transcode, rewrite tags, or delete sources unless the user asked.
9. Do not invent a year or provider ID. Look it up or omit it.
10. Plan the moves, then execute. Never overwrite an existing target.
...
4. **Sanitize** every path component ([shared.md](references/shared.md#forbidden-characters)).
...
| Kind | Pattern |
| --- | --- |
| Subtitles / external audio | `Name.<flags>.<lang>.srt` — see [shared.md](references/shared.md#external-subtitles-and-audio) |
| NFO | `Name.nfo` |
